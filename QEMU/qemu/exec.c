...
#ifdef CUSTOM_MEMORY
    target_ulong _addr = addr*4;
    while (len > 0) {
        page = _addr & TARGET_PAGE_MASK;
        l = (page + TARGET_PAGE_SIZE) - _addr;
        if (l > len)
            l = len;
        if (is_write) {
            /* XXX: this code should not depend on lock_user */
            if (!(p = lock_user(VERIFY_WRITE, _addr, l, 0)))
                return -1;
            if ( (addr > START_PPDL_RAM) && ((addr - START_PPDL_RAM) < sizeCustomRAM) ) {
                memcpy(customRAM + (addr - START_PPDL_RAM)*4, buf, l);
            } else {
                if ( (addr < START_PPDL_RAM) && (addr < sizeCustomMemory) ) {
                    memcpy(customMemory + _addr, buf, l);
                }
            }
            memcpy(p, buf, l);
            unlock_user(p, _addr, l);
        } else {
            if ( (addr > START_PPDL_RAM) && ((addr - START_PPDL_RAM) < sizeCustomRAM) ) {
                memcpy(buf, customRAM + (addr - START_PPDL_RAM)*4, l);
            } else {
                if ( (addr < START_PPDL_RAM) && (addr < sizeCustomMemory) ) {
                    memcpy(buf, customMemory + _addr, l);
                } else {
                    int i;
                    for (i = 0x0; i < l; i++)
                        buf[i] = 0xFF;
...
void *qemu_map_paddr_to_host(hwaddr *paddr_p, int *len)
{
    ...
    MemoryRegionSection *const section = phys_page_find(d->phys_map,
                                                        paddr,
                                                        d->map.nodes,
                                                        d->map.sections);
    char *host_base = NULL;
    ...
    hwaddr addr, const uint8_t *buf, int len, enum write_rom_type type)
{
    ...
    while (len > 0) {
        ...
        section = phys_page_find(d->phys_map, addr,
                                d->map.nodes, d->map.sections);
        ...
        if (!(memory_region_is_ram(mr) ||
            ...
            if(memory_region_is_tlmu_ramd(mr)){
                if(section->offset_within_address_space <= addr
                    ...
                    fprintf(stderr, "start:0x%08llX len:0x%X section start:0x%08llX size:0x%lX\n",
                            (long long) addr, len,
                            (long long) section->offset_within_address_space, int128_get64(section->size)
                            );
                    abort();
                }
            } else {
                                         ...
                                         enum device_endian endian)
{
    ...
    if (memory_region_is_tlmu_ramd(mr) || l < 4 || !memory_access_is_direct(mr, false)) {
        /* I/O case */
                        ...
                        uint8_t *buf, int len, int is_write)
{
    ...
    while (len > 0) {
        ...
        if (l > len)
            l = len;
        phys_addr += (_addr & ~TARGET_PAGE_MASK);
        if (is_write) {
