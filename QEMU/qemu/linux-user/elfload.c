...
#define SIZE_PPDL_RAM 0x1FFFFFFF
#define START_PPDL_RAM 0x20000000

#if defined(CONFIG_LINUX_USER)
#define CUSTOM_MEMORY
#if defined(CUSTOM_MEMORY) && defined(TARGET_PPDL)
#define SIZE_CUSTOM_MEMORY SIZE_PPDL_MEMORY
#define SIZE_CUSTOM_RAM SIZE_PPDL_RAM
extern void *customMemory;
extern uint64_t sizeCustomMemory;
extern void *customRAM;
extern uint64_t sizeCustomRAM;
#endif
#endif

#if defined(TARGET_PPDL)
/* NM6405 (0 for legacy) */
#define EM_NM640X_6405	0
/* NM6403 */
#define EM_NM640X_6403	1
/* NM6405, NMC3 soc, ... */
#define EM_NM640X_NMC3	2
/* NMC3 soc */
#define EM_NM640X_SOC	3
/* NM6407 */
#define EM_NM640X_6407	4
/* NM6407 */
#define EM_NM640X_NMC4	4
/* NM6407 fixed */
#define EM_NM640X_NMC4I	5
/* NM6407 float */
#define EM_NM640X_NMC4F	6
/* NM6407 fixed+float */
#define EM_NM640X_NMC4FI 7
#endif
...
#ifdef TARGET_PPDL
// [TODO] How to parce ppdl binary file
#define ELF_START_MMAP 0x08000000

#define elf_check_arch(x) ((x) == EM_PPDL)

#define ELF_ARCH EM_PPDL
#define ELF_CLASS ELFCLASS32
#define ELF_DATA  ELFDATA2LSB
...
#define ELF_NREG 34 /* [TODO] fix */
typedef target_elf_greg_t target_elf_gregset_t[ELF_NREG];

static void elf_core_copy_regs(target_elf_gregset_t *regs,
                               const CPUPPDLState *env)
{
    // [TODO] Copy regs (*regs)[i] = tswapreg(env->gr[i]);
...
{
    unsigned long /*abi_ulong*/ stack_base, size, error, guard;
    int i;
    ...
    error = (char *) malloc(sizeof(char) * (size+guard) );
    // [TODO] CentOS memory alloc bug
...
#ifdef __CYGWIN__
    #define MAX_MEM_COUNTER 2000
    #define MEM_STEP 0x100000
...
#ifdef __CYGWIN__
        if (mem_counter < MAX_MEM_COUNTER) {
            mem_counter += 0x1;
        } else {
            if (host_size > MEM_STEP)
                host_size -= MEM_STEP;
            else
                return (unsigned long)-1;
        }
                           ...
                           char bprm_buf[BPRM_BUF_SIZE])
{
...
#ifdef CONFIG_USE_FDPIC
    {
        ...
        for (i = 0; i < ehdr->e_phnum; ++i) {
            ...
            case PT_DYNAMIC:
#ifdef TARGET_PPDL
                info->pt_dynamic_addr = phdr[i].p_paddr*4 + load_bias;
#else
            ...
            case PT_LOAD:
#ifdef TARGET_PPDL
                loadsegs->addr = phdr[i].p_paddr*4 + load_bias;
#else
...
#ifdef TARGET_PPDL
    if (info->elf_flags < EM_NM640X_6407) {
        FILE *log_file = fopen("qemu.err.log", "w");
        printf("error: ELF file isn't for NMC4 architecture\n");
        fprintf(log_file, "error: ELF file isn't for NMC4 architecture\n");
        fclose(log_file);
        exit(1);
    }
#endif
...
#if defined(CUSTOM_MEMORY) && defined(TARGET_PPDL)
    sizeCustomMemory = SIZE_CUSTOM_MEMORY;
    customMemory = calloc(sizeCustomMemory, sizeof(uint64_t));
    while (!customMemory) {
        sizeCustomMemory = sizeCustomMemory / 0x10;
        customMemory = calloc(sizeCustomMemory, sizeof(uint64_t));
    }
    sizeCustomRAM = SIZE_CUSTOM_RAM;
    customRAM = calloc(sizeCustomRAM, sizeof(uint64_t));
    while (!customRAM) {
        sizeCustomRAM = sizeCustomRAM / 0x10;
        customRAM = calloc(sizeCustomRAM, sizeof(uint64_t));
    }
#endif
    for (i = 0; i < ehdr->e_phnum; i++) {
...
#ifdef TARGET_PPDL
            vaddr = load_bias + eppnt->p_paddr*4;
#else
...
#if defined(CUSTOM_MEMORY) && defined(TARGET_PPDL)
            if ( ((eppnt->p_paddr > SIZE_PPDL_MEMORY) && (eppnt->p_paddr < START_PPDL_RAM))
                 || ((eppnt->p_paddr > START_PPDL_RAM + SIZE_CUSTOM_RAM)) ) {
                 printf("error: Invalid ELF image for this architecture\n");
                 free(customMemory);
                 free(customRAM);
                 exit(1);
            }
            if (eppnt->p_paddr < START_PPDL_RAM) {
                if (eppnt->p_offset + eppnt->p_filesz <= BPRM_BUF_SIZE) {
                    memcpy(customMemory + eppnt->p_paddr*0x04, bprm_buf + eppnt->p_offset, eppnt->p_filesz);
                } else {
                    uint64_t err = pread(image_fd, customMemory + eppnt->p_paddr*0x04, eppnt->p_filesz, eppnt->p_offset);
                    if (err != eppnt->p_filesz) {
                        memcpy(customMemory + eppnt->p_paddr*0x04, (void *)(GUEST_BASE + eppnt->p_paddr*0x04), eppnt->p_filesz + vaddr_po);
                    }
                }
            } else {
                if (eppnt->p_offset + eppnt->p_filesz <= BPRM_BUF_SIZE) {
                    memcpy(customRAM + (eppnt->p_paddr - START_PPDL_RAM)*0x04, bprm_buf + eppnt->p_offset, eppnt->p_filesz);
                } else {
                    uint64_t err = pread(image_fd, customRAM + (eppnt->p_paddr - START_PPDL_RAM)*0x04, eppnt->p_filesz, eppnt->p_offset);
                    if (err != eppnt->p_filesz) {
                        memcpy(customRAM + (eppnt->p_paddr - START_PPDL_RAM)*0x04, (void *)(GUEST_BASE + eppnt->p_paddr*0x04), eppnt->p_filesz + vaddr_po);
                    }
                }
            }
#endif
