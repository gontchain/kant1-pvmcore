...
DEF("tlmdbg", 0, QEMU_OPTION_tlmdebug, \
    "-tlmdbg\n"
    "                enable TLM debug output\n"
    "                This option is only valid with -M tlm-mach option enabled.\n",
    QEMU_ARCH_ALL)
STEXI
@item -tlmdbg
@findex -tlmdbg
Enable TLM debug output.(default:off)
ETEXI

ARCHHEADING(, QEMU_ARCH_PPDL)

ARCHHEADING(\nTARGET-PPDL options:, QEMU_ARCH_PPDL)
STEXI
@table @option
ETEXI

DEF("rv", 0, QEMU_OPTION_rv, \
    "-rv             change return value of successful end\n", QEMU_ARCH_PPDL)
STEXI
@item -rv
@findex -rv
Change value of gr[7] for successful end
ETEXI

DEF("nosm", 0, QEMU_OPTION_no_semihosting, \
    "-nosm           switch off semihosting\n", QEMU_ARCH_PPDL)
STEXI
@item -nosm
@findex -nosm
Switch off use of semihosting routines
ETEXI

DEF("tick", 0, QEMU_OPTION_tick_enable, \
    "-tick           print total ticks passed at the end of simulation\n", QEMU_ARCH_PPDL)
STEXI
@item -tick
@findex -tick
...
DEF("showtrace", 0, QEMU_OPTION_ppdl_trace, \
    "-showtrace      display trace information\n", QEMU_ARCH_PPDL)
STEXI
@item -showtrace
@findex -showtrace
...
DEF("ctrace", 0, QEMU_OPTION_ppdl_ctrace, \
    "-ctrace      display trace information (old-style)\n"
    "                cannot be combined with -trace and -regtrace flags\n", QEMU_ARCH_PPDL)
STEXI
@item -ctrace
@findex -ctrace
Display program trace (old-style)
ETEXI

DEF("regtrace", HAS_ARG, QEMU_OPTION_regtrace, \
    "-regtrace [,...]\n"
    "                display trace information for specified registers\n", QEMU_ARCH_PPDL)
STEXI
@item -regtrace @var{reglist}
@findex -regtrace
Display program trace for registers listed in @var{reglist}. The @var{reglist} can be set @code{all}
to display any register changes and a single registers list separated by commas (@code{,}):
@example
-regtrace all
-regtrace ar5
-regtrace afifo,gr0,verg7
@end example
ETEXI

DEF("memtrace", HAS_ARG, QEMU_OPTION_memtrace, \
    "-memtrace [,0x...]\n"
    "                display memory trace information for specified addresses\n", QEMU_ARCH_PPDL)
STEXI
@item -memtrace @var{memlist}
@findex -memtrace
Display program memory trace for addresses listed in @var{memlist}. The @var{memlist} can be set @code{all}
to display requests for any address, address range formatted @code{[0x...-0x...]}, a single address formatted @code{[0x...]}
and any combination of the above separated by commas (@code{,}).\n\n
Available address spaces:
@example
1) RAM [ 0x0 - 0xBFFFF ]
2) DDR [ 0x20000000 - 0x3FFFFFFF ]
@end example
Examples:
@example
-memtrace all
-memtrace 0x1F2
-memtrace 0x100-0xBFFFF
-memtrace 0x0,0x100,0x2F0-0x3B0
@end example
ETEXI

DEF("nodebug", 0, QEMU_OPTION_no_debug, \
    "-nodebug        disable to display debug information\n", QEMU_ARCH_PPDL)
STEXI
@item -nodebug
@findex -nodebug
Disable to display debug information.(default:off)
ETEXI

DEF("we", 0, QEMU_OPTION_we, \
    "-we             enable exiting in gdb mode\n", QEMU_ARCH_PPDL)
STEXI
@item -we
@findex -we
...
DEF("fixed", 0, QEMU_OPTION_fixed_insns, \
    "-fixed          translate only fixed point instructions\n", QEMU_ARCH_PPDL)
STEXI
@item -fixed
@findex -fixed
Translate only fixed point instructions
ETEXI

DEF("floating", 0, QEMU_OPTION_float_insns, \
    "-floating       translate only floating point instructions\n", QEMU_ARCH_PPDL)
STEXI
@item -floating
@findex -floating
Translate only floating point instructions
ETEXI
