...
struct ChipMem {
  ...
  uint32 *p_WriteWaitTime;  // массив значений времени задержки при записи в память (если значение равно 0xFFFFFFFF, то память считается долько для чтения)*
  uint32 m_StackStartAddr;  // начальный адрес стека
  uint32 m_EndStackAddr;    // Конечный адрес стека
  uint32 m_Flags;           // массив флагов (флаги определяют, какие ситуации проверять, а какие нет)
  uint32 m_SemiHostFiles;   // максимальное кол-во файлов, которые одновременно могут быть открыты через semihosting
  uint32 m_SemiHostAddr;    // адрес маппирования семихостинга
