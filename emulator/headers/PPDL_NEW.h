...
#define ADD2BUS() MemBusInt->Link(mb_Temp, pm_ChipMem->p_startAddr[i], pm_ChipMem->p_Size[i]);\

template<class TBusType> void InitBusMem(TBusType* MemBusInt,ChipMem * pm_ChipMem, TCycleCount aPipeLenExt =0)
{
  ...
  // 
  if(pm_ChipMem->p_ReadWaitTime == NULL) // no delay information - just  
  {
  ...
  else
  {
    ...
    {
      unsigned long wait_state = pm_ChipMem->p_ReadWaitTime[i];
      unsigned long wait_write = pm_ChipMem->p_WriteWaitTime[i];
      pipe_delay = ((pm_ChipMem->p_startAddr[i]&0x7FFFFFFF)<0x8000) ? 0:aPipeLenExt; 
      // 
      ...
      else
      {
        ...
        {
          TDDramMemBlock<uint32,uint64,uint32>* mb_Temp = new TDDramMemBlock<uint32,uint64,uint32>(pm_ChipMem->p_Size[i],cfg->mPageSize,cfg->mLatency,pipe_delay,cfg->mMulFactor,
