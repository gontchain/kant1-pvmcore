...
#if 0
void *operator new(size_t Size /*,const char *str*/) throw(ELF_ERROR_XALLOC)
{
    const char *str = "libelf";
    char *ptr = 0;
    ...
    if (ptr == 0)  {
        strstream s(_ELF_NEW_STR,255,ios::out);
        s << Size;
        throw ELF_ERROR_XALLOC("Elf error requesting ",s.str(),
                    " bytes memory in ",str);
    }
#if _DEBUG && defined(NEW_TRACE)
    cout << "New: allocate at " << (void *)ptr << " " << str << endl;
#endif
    return (void *)(ptr);
}

//  Microsoft Visual C++ 4.0 не понимает оператора new для массивов
...
void *operator new[](size_t Size/*, const char *str*/) throw(ELF_ERROR_XALLOC)
{
    ...
    // _DEBUG && defined(_MSC_VER) && defined(_AFX_DEBUG_INCLUDE)
...
void CheckFile(int aID)
{
  fstream fstr;
  std::iostream* check_str = &fstr;
  fstr.open("test.elf" , ios::out | ios::trunc | ios::binary);
...
Elf32_Phdr *ELF::GetPhdr() throw(ELF_ERROR)
{
    ...
    //    if (!(*pfile))  throw (ELF_ERROR_FILE("Elf error reading file",filename));
        return phdr;
    }
    catch(ELF_ERROR) {
            delete phdr;
...
ELF_SCN * ELF::NewSHSTRTAB() throw(ELF_ERROR)
{
    if ( ehdr->e_shstrndx != 0 )
        throw ELF_ERROR_EXIST("Elf error - ","section \".shstrtab\" already exist");
