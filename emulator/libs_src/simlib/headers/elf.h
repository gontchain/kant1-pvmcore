 ...
 *
 *******************************************************************/
#ifndef ELF_LOADER_CLASS
#define ELF_LOADER_CLASS
...
public:
  ...
  ELF_DATA *XlateToF(ELF_DATA *dest,unsigned int encode) throw(ELF_ERROR);

  ELF_DATA *XlateToM(ELF_DATA *dest,unsigned int encode) throw(ELF_ERROR);
  unsigned int SetFlag(unsigned int f);
  unsigned int ClearFlag(unsigned int f);
};
...
private:
  ...
  Elf32_Word IntSearchSymbolBind( const char * Name, unsigned char Bind ) throw(ELF_ERROR);

public:
  // конструктор: загружает elf файл с именем *name
