...
{
	IsOutCode = 1;
	IsWarnFlags = (1<<MAX_NUM_WARNINGS)-1 ; // 
	IsShowData = 1;
	CheckResult = 0;
	IsRusLang = 1;
	BuildProfile = 0;
...
int Init()
{
	...
	if(ConfigFileName!=NULL)
	{
		...
		try
		{
			Device = InitChipMem(ConfigFileName,IsWarnFlags|(IsRusLang<<31)|(BuildProfile<<29)); //
		}
	...
	{
    Device = InitSystem((void*)BuildProfile);
	}
...
int RunSimulationNoOut(char* aSimName,uint32 aWait)
{
	...
	do
  {
		...
		Device->Start();
#ifdef WIN32
		while(Device->IsRun()) Sleep(500); // пол секунды
#else
		while(Device->IsRun()) sleep(1);
#endif
		PRINT_LOG("THREAD:stop execution\n");

		cur_pc = Device->GetPc(0);
		if(ProcessWarns(cur_pc)==0)
			return -1;
	}while(cur_pc!=end_pc);
...
char usage_text[] = "usage: cemu [keys] [-bsp bsp_file_name]simulator_lib_name executable file name\n"\
	"keys:\n"\
...
int main(int argc,char* argv[])
{
	...
	if(argv[sim_args_start+2]!=NULL)
	{
		...
		if(!wait_time)
		{
			wait_time = 100; // wait for 10 000 cycles
